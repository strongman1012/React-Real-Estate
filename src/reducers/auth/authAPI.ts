...
export const loginAPI = async (credentials: LoginCredentials): Promise<AuthResponse> => {
    const response = await apiClient.post<AuthResponse>('/login', { ...credentials, application: "Application B" });
    return response.data;
};

export const loginWithTokenAPI = async (token: string): Promise<AuthResponse> => {
    const response = await apiClient.post<AuthResponse>('/loginWithToken', { token: token, application: "Application B" });
    return response.data;
};
...
export const logoutAPI = async (): Promise<{ message: string }> => {
    const response = await apiClient.post('/logout', { application: "Application B" });
    return response.data;
};
